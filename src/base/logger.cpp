...
std::string EscapeJson(const char* input) {
  std::string out;
  for (const unsigned char* p = reinterpret_cast<const unsigned char*>(input ? input : ""); *p;
       ++p) {
    switch (*p) {
    case '\\':
      out += "\\\\";
...
std::string JsonLine(CVSDK_LogLevel level, const char* module, const char* message) {
  ...
  out << "{\"ts\":\"" << std::put_time(&local, "%Y-%m-%dT%H:%M:%S") << '.' << std::setfill('0')
      << std::setw(3) << ms << "\",\"level\":\"" << LevelName(level) << "\",\"module\":\""
      << EscapeJson(module) << "\",\"message\":\"" << EscapeJson(message)
      << "\",\"sdk_version\":\"0.1.0\"}";
  return out.str();
}
} // namespace
...
Logger::Logger() : impl_(new Impl) {
  impl_->worker = std::thread([this] {
    for (;;) {
      ...
      uint64_t max_bytes;
      uint32_t max_files;
      CVSDK_LogCallback callback;
      void* user_data;
      {
        std::unique_lock<std::mutex> lock(impl_->mutex);
        impl_->cv.wait(lock, [this] { return impl_->stopping || !impl_->queue.empty(); });
        if (impl_->stopping && impl_->queue.empty())
          return;
        entry = std::move(impl_->queue.front());
        impl_->queue.pop_front();
        path = impl_->file_path;
        max_bytes = impl_->max_file_bytes;
        max_files = impl_->max_rotated_files;
        callback = impl_->callback;
        user_data = impl_->user_data;
      }
      if (!path.empty()) {
        ...
        if (!error && max_bytes && current_size + entry.json.size() + 1 > max_bytes) {
          for (uint32_t i = max_files; i > 0; --i) {
            const std::string from = i == 1 ? path : path + "." + std::to_string(i - 1);
      ...
      if (callback)
        callback(entry.level, entry.json.c_str(), user_data);
...
CVSDK_Status Logger::Configure(const CVSDK_LogOptions* options) {
  if (options && options->struct_size < sizeof(CVSDK_LogOptions)) {
    SetLastError("invalid CVSDK_LogOptions");
    return CVSDK_INVALID_ARGUMENT;
  }
  if (options && (options->min_level < CVSDK_LOG_TRACE || options->min_level > CVSDK_LOG_OFF)) {
    SetLastError("invalid log level");
    return CVSDK_INVALID_ARGUMENT;
  }
  std::lock_guard<std::mutex> lock(impl_->mutex);
  ...
  impl_->callback = options ? options->callback : nullptr;
  impl_->user_data = options ? options->user_data : nullptr;
  return CVSDK_OK;
}
void Logger::Log(CVSDK_LogLevel level, const char* module, const char* message) {
  if (level < CVSDK_LOG_TRACE || level >= CVSDK_LOG_OFF || !module || !message)
    return;
  std::lock_guard<std::mutex> lock(impl_->mutex);
  ...
  if (level <= CVSDK_LOG_WARN) {
    ...
    if (rate.last.time_since_epoch().count() != 0 && now - rate.last < std::chrono::seconds(1)) {
      ++rate.suppressed;
      return;
    }
    if (rate.suppressed) {
      if (impl_->queue.size() < impl_->queue_capacity) {
        impl_->queue.push_back({level, JsonLine(level, module,
                                                ("previous identical messages suppressed=" +
                                                 std::to_string(rate.suppressed))
                                                    .c_str())});
        ++impl_->accepted;
      } else {
        ++impl_->dropped;
  ...
  impl_->queue.push_back({level, JsonLine(level, module, message)});
  ++impl_->accepted;
  impl_->cv.notify_one();
}
CVSDK_Status Logger::GetStats(CVSDK_LogStats* stats) {
  if (!stats || stats->struct_size < sizeof(CVSDK_LogStats)) {
    SetLastError("invalid CVSDK_LogStats");
    return CVSDK_INVALID_ARGUMENT;
  }
  std::lock_guard<std::mutex> lock(impl_->mutex);
  stats->accepted_count = impl_->accepted;
  stats->dropped_count = impl_->dropped;
  stats->queued_count = static_cast<uint32_t>(impl_->queue.size());
  return CVSDK_OK;
}
} // namespace cvsdk
